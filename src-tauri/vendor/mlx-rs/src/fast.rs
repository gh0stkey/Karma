...
impl ScaledDotProductAttentionMask<'_> {
    fn as_mode_and_masks(&self) -> (&'static CStr, mlx_sys::mlx_array) {
        match self {
            ScaledDotProductAttentionMask::Array(mask) => (DEFAULT_MASK_MODE, mask.as_ptr()),
            ScaledDotProductAttentionMask::Arrays(masks) if masks.len() == 1 => {
                (DEFAULT_MASK_MODE, masks[0].as_ptr())
            }
            ScaledDotProductAttentionMask::Arrays(_) => {
                panic!("multiple attention masks are not supported by mlx-c 0.6 API")
            }
