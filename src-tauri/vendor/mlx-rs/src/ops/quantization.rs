...
pub fn quantize_device(
    ...
    let group_size = mlx_sys::mlx_optional_int {
        value: group_size.into().unwrap_or(64),
        has_value: true,
    };
    let bits = mlx_sys::mlx_optional_int {
        value: bits.into().unwrap_or(4),
        has_value: true,
    };
    let mode = c"affine";

    let vec = <VectorArray as Guarded>::try_from_op(|res| unsafe {
    ...
    let mut values = vec.try_into_values::<Vec<Array>>()?;
    let biases = values.pop().unwrap();
    let scales = values.pop().unwrap();
    let quantized = values.pop().unwrap();
...
pub fn dequantize_device(
    ...
    let mode = c"affine";
    let dtype = mlx_sys::mlx_optional_dtype {
        value: 0,
        has_value: false,
    };
